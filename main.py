...
def predict_eye_state(model, image):
    ...
    # print(prediction)
    # if 0 < prediction[1] <= 0.5:
    #     return 1
    # elif 0.5 < prediction[1] <= 0.6:
    #     return 2
    # elif 0.6 < prediction[1] <= 0.7:
    #     return 3
    # elif 0.7 < prediction[1] <= 0.8:
    #     return 4
    # elif 0.8 < prediction[1] <= 1.0:
    #     return 5

    return round(max(np.random.uniform(low=0.15, high=0.3), prediction[1] - 0.4), 2)
...
while(True):
    ...
    if len(face_locations):
        ...
        left_eye = predict_eye_state(model=model, image=left_eye)
        right_eye = predict_eye_state(model=model, image=right_eye)

        if right_eye != 1 and right_eye != 5:
            left_eye = right_eye
        ...
        cv2.putText(frame, f"Eye state {left_eye}", (x1, y1 - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 0, 0), 2)
